...
foreach ($allPosts as $post) {
    ...
    <div class="col">
        <div class="card">
            ...
            <div class="card-body">
                <a href="<?php echo $router->generate('post', ['id' => $post['_id']]); ?>"
                ...
                if ($post['password'] != '') { ?>
                    <!-- Need to add to i18n -->
                    <p class="card-text text-muted">This post is
                        protected<br><?php echo date('F j, Y', $post['date']); ?></p>
                    <?php
                } else { ?>
                    <p class="card-text text-muted">Posted by <?php echo $post['author']; ?>
                        <br><?php echo date('F j, Y', $post['date']); ?></p>
                <?php } ?>
            </div>
        </div>
    </div>
