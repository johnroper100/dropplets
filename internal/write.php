...
?>
    ...
    <form method="post" enctype="multipart/form-data" action="<?php if (isset($post['title'])) {
        ...
        <fieldset>
            <input type="text" name="blogPostTitle" class="blogPostTitle"
                   placeholder="<?php i18n("write_post_title_placeholder"); ?>" required
                   value="<?php if (isset($post['title'])) {
                       echo $post['title'];
                   } ?>"/>
            <input type="text" name="blogPostAuthor" class="blogPostAuthor"
                   placeholder="<?php i18n("write_post_author_placeholder"); ?>" required
                   value="<?php if (isset($post['author'])) {
                       echo $post['author'];
                   } ?>"/>
            <input type="url" name="blogPostImageURL" class="blogPostImageURL"
                   placeholder="<?php i18n("write_post_image_placeholder"); ?>"
                   value="<?php if (isset($post['image'])) {
            ...
            <?php } ?>
            <input type="text" name="blogPostPassword" class="blogPostTitle"
                   placeholder="<?php i18n("write_post_password_placeholder"); ?>"
                   value="<?php if (isset($post['password'])) {
                       echo $post['password'];
                   } ?>"/>
            <textarea name="blogPostContent" id="blogPostContent"
                      placeholder="<?php i18n("write_post_markdown_placeholder"); ?>"><?php if (isset($post['content'])) {
                    echo $post['content'];
                } ?></textarea>
        </fieldset>
        <?php if (isset($post['title'])) { ?>
